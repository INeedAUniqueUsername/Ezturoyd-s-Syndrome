...
public class Weapon {
	...
	public Weapon(double posAngle, double posRadius, double fire_angle_offset, int fire_cooldown_time,
			int projectile_speed, int projectile_damage, int projectile_lifetime) {
	...
	public void draw(Graphics g) {
		...
		/*
		 * g.setColor(Color.RED); double trajectoryAngle =
		 * SpaceObject.arctanDegrees( projectile_speed *
		 * SpaceObject.sinDegrees(fire_angle) + owner.getVelY(),
		 * projectile_speed * SpaceObject.cosDegrees(fire_angle) +
		 * owner.getVelX() );
		 * 
		 * Point2D.Double projectile_end = owner.polarOffset(trajectoryAngle,
		 * getProjectileRange()); g.drawLine((int) pos_x, (int) pos_y, (int)
		 * projectile_end.getX(), (int) projectile_end.getY());
		 */
	}
